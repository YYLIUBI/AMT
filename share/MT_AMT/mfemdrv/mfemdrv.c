...
//#define SWITCHHP_PIN         AT91_PIN_PB16//E18//A   ????
//#define SWITCHLP_PIN         AT91_PIN_PB15//E17//A   ???? 
...
static void IO_init(void)
{
	...
	//at91_set_gpio_output(MODE_SEL1,0);
	//at91_set_gpio_output(MODE_SEL2,0);
	...
	//at91_set_gpio_value(MODE_SEL1,0);
	//at91_set_gpio_value(MODE_SEL2,0); 
...
{
	int status;
	status = at91_get_gpio_value(LED1_PIN);
...
{ 	
  switch(cmd)
	...
	case CMD_SETGAINH:
						...
						break;
	/*case CMD_SETACDC:
						at91_set_gpio_value(SWITCHHP_PIN,arg%2);
						at91_set_gpio_value(SWITCHLP_PIN,arg/2);
						break;*/
	case CMD_SETCAL:
...
static  int __init at91_mfem_init(void)
{
	...
	printk("Register MFEM driver for SAM9G45 OK! 2018-11-24\n");
	#ifdef DEBUG
...
{  
	...
	at91_set_gpio_value(SSC_START_PIN, 0); 
 	...
 	unregister_chrdev(MFEMDEV_MAJOR, mfemdev_name);   //ºÍÔ­À´µÄ²»Ò»ÑùÁË£¬Ã»ÓÐ·µ»ØÖµ  --HJJ
  	printk(KERN_INFO"Unregister MFEM driver ; 2018-11-24\n"); 
}  
