...
data={};
data_in={};
% fs={};fin={};fn={};LEN={};LEN_pass={};T={};fdelta={};filename_out={};
figure(1);
for a=1:1:length(FileName)    
    ...
    case 'H'
        ...
        fdelta(a)=LEN(a)*fin(a)/fs(a);
        ts(a)=3;
        fseek(fid,16+42*32,'bof');%skip 16B
    case 'M'
        fs(a)=150;
        fin(a)=2.5;
        fn(a)=14;
        ...
        fdelta(a)=LEN(a)*fin(a)/fs(a);
        ts(a)=4;
        fseek(fid,16+14*32,'bof');%skip 16B
    case 'L'
        fs(a)=15;
        fin(a)=15/160;
        fn(a)=36;
        ...
        fdelta(a)=LEN(a)*fin(a)/fs(a);  
        ts(a)=5;
        fseek(fid,16+72*32,'bof');%skip 16B
    end 
...
for a=1:1:length(FileName)
    ...
    subplot(211);
    loglog(f_out{a},d_amp{1+8*(a-1)}/16,'r^-');hold on;
    loglog(f_out{a},d_amp{3+8*(a-1)}/16,'b^-');hold on;
    loglog(f_out{a},d_amp{2+8*(a-1)}/16,'g^-');hold on;
    loglog(f_out{a},d_amp{5+8*(a-1)}/4,'k*-');hold on;
    loglog(f_out{a},d_amp{6+8*(a-1)}/4,'y*-');hold on;
    loglog(f_out{a},d_amp{7+8*(a-1)}/4,'m*-');grid on;  
    xlabel('f,Hz');
    ylabel('Gain,V/V');
    legend('Ex','Ey','Ez','Hx','Hy','Hz');
...
fidri=fopen([FileName{a}(1:4),'L.asc'],'w');
% for a=1:1:length(FileName)    
%     for j=1:1:fn(a)
...
%             fprintf(fidri,'%8.5f,%8.5f,%8.5f,%8.5f,%8.5f,%8.5f,',...
...
for k=1:1:length(FileName)
    ...
    for j=1:1:fn(a)
        fprintf(fidri,'%d, %010.5f, %02d,',ts(a),f_out{a}(j),2*j-1);
        for i=1:1:3
            d_real{i+8*(a-1)}(j)=(realA{i+8*(a-1)}(j)*realB{a}(j)+imageA{i+8*(a-1)}(j)*imageB{a}(j))/(realB{a}(j)^2+imageB{a}(j)^2)/16;
            d_image{i+8*(a-1)}(j)=(imageA{i+8*(a-1)}(j)*realB{a}(j)-realA{i+8*(a-1)}(j)*imageB{a}(j))/(realB{a}(j)^2+imageB{a}(j)^2)/16;
            fprintf(fidri,'% 4.5f,% 4.5f,% 4.5f,% 4.5f,% 4.5f,% 4.5f,',...
            realA{i+8*(a-1)}(j),imageA{i+8*(a-1)}(j),realB{a}(j),imageB{a}(j),d_real{i+8*(a-1)}(j),d_image{i+8*(a-1)}(j));        
        end
        for i=5:1:7
            d_real{i+8*(a-1)}(j)=(realA{i+8*(a-1)}(j)*realB{a}(j)+imageA{i+8*(a-1)}(j)*imageB{a}(j))/(realB{a}(j)^2+imageB{a}(j)^2)/4;
            d_image{i+8*(a-1)}(j)=(imageA{i+8*(a-1)}(j)*realB{a}(j)-realA{i+8*(a-1)}(j)*imageB{a}(j))/(realB{a}(j)^2+imageB{a}(j)^2)/4;
            fprintf(fidri,'% 4.5f,% 4.5f,% 4.5f,% 4.5f,% 4.5f,% 4.5f,',...
            realA{i+8*(a-1)}(j),imageA{i+8*(a-1)}(j),realB{a}(j),imageB{a}(j),d_real{i+8*(a-1)}(j),d_image{i+8*(a-1)}(j));        
        end
        fprintf(fidri,'\n');
