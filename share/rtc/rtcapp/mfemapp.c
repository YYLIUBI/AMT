...
int set_alarm(int mday,int hour,int min,int sec) 
{
  ...
  u8 cs = RTC_ACQ_CS;
	...
	alarmdata[0] = BIN2BCD(sec);
	alarmdata[1] = BIN2BCD(min);
	alarmdata[2] = BIN2BCD(hour);
	alarmdata[3] = BIN2BCD(mday);
	fd = open_mfem();
	ioctl(fd,CMD_ACCESS_SPI,&cs);
	ioctl(fd,CMD_ALARM_INIT,alarmdata);
	ioctl(fd,CMD_RELEASE_SPI,&cs);
	close(fd);
...
//GPStime[21]="06-09-03 02:48:11.19\n";
	...
	pgps->gpstime.tm_hour=(GPStime[9]-0x30)*10+(GPStime[10]-0x30);
  	...
  	pgps->gpstime.tm_year=(GPStime[0]-0x30)*10+(GPStime[1]-0x30);
  	sprintf(time , "%02d%02d%02d%02d%02d%02d",
												pgps->gpstime.tm_hour,
												pgps->gpstime.tm_min,
												pgps->gpstime.tm_sec,
            									pgps->gpstime.tm_mday,
            									pgps->gpstime.tm_mon+1,
            									pgps->gpstime.tm_year);
  	pgps->gpstime.tm_wday = weekcal(time);
...
int main(int argc,char *argv[])
{
	...
	fd = open_mfem();
	while((cmd = getopt(argc,argv, "rsgab"))!= -1)
	{
		switch(cmd)
		{
			case 'r':       
            ...
            case 'a':   
            			...
            			printf("wday:%02d\n",pgpsinfo->gpstime.tm_wday);
            			ioctl(fd, CMD_SETGAIN,0);
						//printf("arg_gain_e:%d\n",arg_gain_e);
						ioctl(fd, CMD_SETGAINH,0);
						//printf("arg_gain_h:%d\n",arg_gain_h);
            ...
            case 'b':
            			...
            			set_alarm(pgpsinfo->gpstime.tm_mday,pgpsinfo->gpstime.tm_hour,
            				pgpsinfo->gpstime.tm_min+1,pgpsinfo->gpstime.tm_sec);
            			break;
        default:
			Usage(EXIT_SUCCESS);
			break;
